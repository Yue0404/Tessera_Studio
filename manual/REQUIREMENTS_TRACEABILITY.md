...
| Civilization VI                                                                  | conditional | 真实安装已验证 209 元素/114 资源；CIV-009 有 DomainGroup 直接实现、测试和生产 E2E；Win11 25H2 x64 自包含 GUI 实机及 `extractor-v0.1.0-preview.1` 正式 Release/catalog 已闭合；无游戏或无包仍不阻塞核心网站 |
| VIEW-008                                                                         | covered     | 真实分块 P95、缺帧、接缝与跨块淘汰回归                                                                                                                                                                     |
| 最大地图内存与跨块稳定                                                           | covered     | 强制 GC、LRU 饱和及线/箭头/覆盖物往返                                                                                                                                                                      |
| PERF-010 参考档性能                                                              | blocked     | 严格 CIM/Schema/场景、系统 Edge、被测提交 provenance/release:ready 门禁已实现；Issue #12 仍需冻结参考机实跑及决定是否扩展浏览器范围；所有者已接受证据延期风险，不等于 covered                              |
| PERF-002 视口操作延迟                                                            | blocked     | 当前 1440×900、DPR=1 补充数据通过但 `comparable=false`；仍须在 PERF-010 冻结参考硬件闭合；所有者已接受证据延期风险，不等于 covered                                                                         |
| 当前及前一主要浏览器版本                                                         | blocked     | Issue #12；Edge 151/150 最终有界 A/B 的失败跨版本与轮次漂移，不能归因于 Edge 150；停止继续加时，所有者已接受证据延期风险，不等于 covered                                                                   |
...
| 项目许可证                                                                       | covered     | 根目录 `LICENSE`：官方未修改的 PolyForm Noncommercial License 1.0.0，含 Required Notice，且仅覆盖 Tessera Studio 自有代码和资产                                                                            |
